...
async fn get_single_article_has_self_link_and_type() {
    ...
    let res = app
        .send(TestRequest::get("/articles/art-01").accept_json_api().build())
        .await;
    let res = res.assert_status(StatusCode::OK).assert_json_api_content_type();
    assert_eq!(res.json()["data"]["type"], "articles");
    assert_eq!(res.json()["data"]["id"], "art-01");
    assert_eq!(res.json()["links"]["self"], "http://api.test/articles/art-01");
}
